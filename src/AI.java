...
public class AI {
	...
	/*public Cell sortList(List<Cell> list) {
		int max = 0;
		Cell tmp = null;
		for (Cell cell : list) {
			if (this.getMap().get(Character.valueOf(cell.getValue())).size() > max) {
				tmp = cell;
				max = this.getMap().get(Character.valueOf(cell.getValue())).size();
			}
		}
		return tmp;
	}*/
